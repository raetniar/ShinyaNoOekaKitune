...
function copyCommonTag(tagText) {
    ...
    if (typeof insertYtTagToFocusedInput === 'function') {
        try {
            insertYtTagToFocusedInput(copyString);
        } catch(e) {}
    }
...
window.copyCommonTag = copyCommonTag;
        ...
        function setLanguage(lang, shouldSave = true) {
            ...
            if (tn) {
                tn.innerHTML = '';
                const defaultTids = ['main-tab', 'raid-tab', 'twitch-tab', 'youtube-tab', 'cmd-tab', 'friends-tab', 'cp-tab', 'memo-tab', 'misc-tab'];
        ...
        function _buildFriendCard(f, ci, fi, L, I, groupTags, sortMeta) {
            ...
            <div class="record-body">
                ...
                <span class="field-label" style="color:#ff0000; font-weight:bold;">YouTube ID / ハンドル名 (例: @uikouka)</span>
                <input type="text" id="f-yt-id-${ci}-${fi}" value="${raidSoEscape(f.youtubeId || '')}" placeholder="例: @uikouka または チャンネルID" oninput="updateFriendField(${ci}, ${fi}, 'youtubeId', this.value)">
